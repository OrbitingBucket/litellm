...
class TomlDocument:
    ...
    def set_value(self, section: str, key: str, value: object) -> None:
        ...
        if section == ROOT_SECTION or not isinstance(value, Mapping):
            ...
            return
        self._set_child_table(container, section, key, value)

    def _set_child_table(
        self,
        container: tomlkit.TOMLDocument | Table | InlineTable | OutOfOrderTableProxy,
        section: str,
        key: str,
...
def _json_value(value: object) -> JsonValue:
    ...
    root: Final[dict[str, JsonValue] | list[JsonValue]] = (
        {} if isinstance(value, Mapping) else []
    )  # mutable-ok: JSON document under construction
    pending: Final[list[tuple[object, dict[str, JsonValue] | list[JsonValue], int]]] = [
        (value, root, 0)
    ]  # mutable-ok: worklist
    while pending:
        ...
        if depth > _MAX_CONFIG_DEPTH:
            raise AgentConfigError(
                f"The config nests more than {_MAX_CONFIG_DEPTH} levels deep; cannot proceed safely."
            )
        entries = source.items() if isinstance(source, Mapping) else enumerate(source)  # pyright: ignore[reportUnknownVariableType]  # Mapping | list | tuple narrowed by the isinstance guards above
        for key, item in entries:
            if isinstance(item, (Mapping, list, tuple)):
                child: dict[str, JsonValue] | list[JsonValue] = (
                    {} if isinstance(item, Mapping) else []
                )  # mutable-ok: JSON document under construction
                pending.append((item, child, depth + 1))
                converted: JsonValue = child
            else:
                converted = _scalar_json(item)
...
def _toml_item(value: object, inline: bool = False) -> object:
    ...
    if not isinstance(value, Mapping):
        return value
    root: Final = tomlkit.inline_table() if inline else tomlkit.table()
    pending: Final[list[tuple[Mapping[object, object], Table | InlineTable, bool, int]]] = [
        (value, root, inline, 0)
    ]  # mutable-ok: worklist
    while pending:
...
def unconfigure_document(
    ...
    target: Final = write_target(path)
    try:
        if document.is_empty() and not receipt.file_existed:
        ...
        raise AgentConfigError(f"Could not write {target}: {e}") from e
    return UnconfigureOutcome(restored=tuple(restored), kept=tuple(kept), withheld=tuple(withheld))
