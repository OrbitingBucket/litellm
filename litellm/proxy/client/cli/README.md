...
Plain `lite configure`, with no agent named, asks the same things interactively: which agents to wire (Claude Code and Codex), which of the proxy's models each starts on, picked from `/v1/models` with a type-to-filter prompt, and whether to start one right away. `configure claude` honors `CLAUDE_CONFIG_DIR` the way Claude Code does, so a relocated config directory gets the patch rather than `~/.claude`

#### Configuring Codex the Same Way

`lite configure codex [--api-key sk-...] [--model <id>]` does for Codex, the CLI and the desktop app, what `configure claude` does for Claude Code, and `lite unconfigure codex` puts it back. Codex ignores `OPENAI_BASE_URL` and routes through a named provider, so the command edits `~/.codex/config.toml` (or `$CODEX_HOME/config.toml`) in place: a `[model_providers.litellm]` table on the HTTP/SSE Responses transport, `model_provider = "litellm"`, and, with `--model`, that model plus its context window from the proxy as `model_context_window`, since Codex sizes its context from its own catalog and knows nothing about proxy models. Comments and every other key and table survive the rewrite. A Codex started from the desktop app never sees your shell environment, so the credential is not an env var: a virtual key is written into the table as `experimental_bearer_token`, and your `lite login` becomes a `[model_providers.litellm.auth]` command that Codex runs itself (`lite --base-url ... auth print-token`), so a later login renews it. Codex's `/model` picker keeps showing its built-in catalog; to switch to another proxy model, run `configure codex --model` again or pass `codex -m <id>`

Both `configure` subcommands take `--launch` to start the agent right after wiring it, and plain `lite configure` ends by asking whether to start one of the agents it just configured
