...
class TestTeamModelUpdate:
    ...
    async def test_team_move_releases_the_public_name_from_the_source_team(self, moved_row, source_backing, released):
        ...
        assert (source_team.models if source_team is not None else ["shared-name"]) == (
            ["no-default-models"] if released else ["shared-name"]
        )
    ...
    # Deleting a team's last deployment used to write `team.models = []`, which the access check
    # reads as every model; the shrink owner leaves a list that grants nothing instead.
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "current_models, expected",
        [(["only-model"], ["no-default-models"]), (["only-model", "gpt-4o"], ["gpt-4o"])],
    )
    async def test_deleting_the_last_team_deployment_never_opens_the_team(self, current_models, expected):
        from litellm.proxy.common_utils.user_api_key_cache import UserApiKeyCache
        from litellm.proxy.management_endpoints.model_management_endpoints import _remove_unbacked_team_models
        ...
        deleted = Deployment(
            ...
            model_info=ModelInfo(team_id="team_a", team_public_model_name="only-model"),
        )
        ...
        prisma = MagicMock()
        prisma.db.litellm_teamtable.find_unique = AsyncMock(
            return_value=LiteLLM_TeamTable(team_id="team_a", models=list(current_models))
        )
        prisma.db.litellm_teamtable.update = AsyncMock(side_effect=update)
        prisma.db.litellm_proxymodeltable.find_many = AsyncMock(return_value=[])
        prisma.db.litellm_modeltable.find_many = AsyncMock(return_value=[])
        ...
        cached_team = await cache.async_get_cache(key="team_id:team_a", model_type=LiteLLM_TeamTableCachedObj)
        assert cached_team is not None and cached_team.models == expected
